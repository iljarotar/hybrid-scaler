...
func HybridOpposite(s *strategy.State, cpuLimitsToRequestsRatio, memoryLimitsToRequestsRatio *inf.Dec) (*strategy.ScalingDecision, error) {
	horizontalRecommendation, err := calculateDesiredReplicas(s)
	if err != nil {
		return nil, err
	}
	...
	var opposite *inf.Dec
	if difference.Cmp(zero) > 0 {
		opposite = new(inf.Dec).Add(minReplicas, new(inf.Dec).Neg(currentReplicas))
		opposite.QuoRound(opposite, difference, 0, inf.RoundDown)
	} else if difference.Cmp(zero) < 0 {
		opposite = new(inf.Dec).Add(currentReplicas, new(inf.Dec).Neg(maxReplicas))
		opposite.QuoRound(opposite, difference, 0, inf.RoundDown)
	} else {
		// if the difference is zero, then the unrounded value was a small negative one, so we can try to remove replicas and adjust the resources accordingly
		opposite = inf.NewDec(-1, 0)
	}

	desiredReplicas := new(inf.Dec).Add(currentReplicas, opposite)
	limitedReplicas := limitValue(desiredReplicas, minReplicas, maxReplicas)
